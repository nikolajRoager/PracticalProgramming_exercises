...
public static class ode
{
    ...
    )
    {
        ...
        while (x<xf)
        {
            ...
            bool reject = false;
            bool first_loop = true;//If this is the first time we have tried this step, then we should always accep the next suggested h, even if it is larger than what we have now
            do
            {
                reject = false;
                doublelist Data5;
                ...
                for (int i = 0; i<Data.size ; ++i)
                {
                    ...
                    if (Error[i]!=0)
                    {
                        ...
                        //If this is the worst parameter so far, or if this is the first attempt at finding a new h , resize to fit that
